    ...
    def start(self):
        ...
        if inputs:
            ...
            sessions = list(con.execute('SELECT session, start, end, num_cmds, remark FROM '
                                        'visits WHERE session >= ?', (first_session,)))
        con.close()
        ...
        new_db.execute("""CREATE TABLE IF NOT EXISTS visits (session integer
        ...
        if inputs:
            with new_db:
                ...
                new_db.executemany('insert into visits values (?,?,?,?,?)', sessions)
